...
void saveToDotFile(int parent[], int dist[], int graph[V][V], char *vertex[], int src, int dest) {
    ...
    // Track shortest path edges
    int shortestPathEdges[V][V] = {0};
    int v = dest;
    while (parent[v] != -1) {
        shortestPathEdges[parent[v]][v] = 1;
        shortestPathEdges[v][parent[v]] = 1;  // Make it undirected
        v = parent[v];
    }

    // Add edges with conditional coloring
    for (int i = 0; i < V; i++) {
        for (int j = i + 1; j < V; j++) {
            if (graph[i][j] != 0 && graph[i][j] != INF) {
                if (shortestPathEdges[i][j]) {
                    // Shortest path edge in red with thicker line
                    fprintf(f, "    \"%s\" -- \"%s\" [color=red, penwidth=2.0, label=\"%d km\"];\n", vertex[i], vertex[j], graph[i][j]);
                } else {
                    // Regular edge in black
...
int main() {
    ...
    printf("Enter the destination vertex name (e.g., 'Hello Pet Cares','Pet Animal Hospital' etc.): ");
    scanf(" %[^\n]%*c", destName);
